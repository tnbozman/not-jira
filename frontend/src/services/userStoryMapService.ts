...
export const userStoryMapService = {
  ...
  async getEpic(projectId: number, themeId: number, epicId: number): Promise<Epic> {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/themes/${themeId}/epics/${epicId}`, {
      headers: await getHeaders(),
    })
    if (!response.ok) {
  ...
  async getStory(projectId: number, epicId: number, storyId: number): Promise<Story> {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/epics/${epicId}/stories/${storyId}`, {
      headers: await getHeaders(),
    })
    if (!response.ok) {
  ...
  async updateStory(projectId: number, epicId: number, storyId: number, story: Story): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/epics/${epicId}/stories/${storyId}`, {
      method: 'PUT',
      headers: await getHeaders(),
      body: JSON.stringify(story),
  ...
  async getSpike(projectId: number, epicId: number, spikeId: number): Promise<Spike> {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/epics/${epicId}/spikes/${spikeId}`, {
      headers: await getHeaders(),
    })
    if (!response.ok) {
  ...
  async updateSpike(projectId: number, epicId: number, spikeId: number, spike: Spike): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/epics/${epicId}/spikes/${spikeId}`, {
      method: 'PUT',
      headers: await getHeaders(),
      body: JSON.stringify(spike),
