...
async function loadBacklog() {
  ...
  error.value = "";
  try {
    backlog.value = await backlogService.getBacklog(props.projectId, filters.value);
  } catch (err: any) {
...
async function loadTeams() {
  try {
    teams.value = await backlogService.getTeams(props.projectId);
  } catch (err) {
...
async function loadReleases() {
  try {
    releases.value = await backlogService.getReleases(props.projectId);
  } catch (err) {
...
async function loadEpics() {
  try {
    const themes = await backlogService.getThemes(props.projectId);
...
async function loadProjectMembers() {
  try {
    const project = await backlogService.getProject(props.projectId);
...
function openPlanning(sprint: any) {
  ...
  backlogService
    .getSprintPlanning(props.projectId, sprint.sprintId)
    .then((data) => {
      planningData.value = {
        planningOneNotes: data.planningOneNotes || "",
        teamPlannings: data.teamPlannings || [],
      };
    })
...
async function savePlanning() {
  try {
    await backlogService.saveSprintPlanning(
      props.projectId,
...
function openReview(sprint: any) {
  selectedSprint.value = sprint;
  backlogService
    .getSprint(props.projectId, sprint.sprintId)
    .then((data) => {
...
async function saveReview() {
  try {
    await backlogService.saveSprintReview(
      props.projectId,
      selectedSprint.value.sprintId,
      reviewNotes.value,
    );
...
function openRetro(sprint: any) {
  ...
  backlogService
    .getSprint(props.projectId, sprint.sprintId)
    .then((data) => {
      retroNotes.value = data.retroNotes || "";
    })
...
async function saveRetro() {
  try {
    await backlogService.saveSprintRetro(
      props.projectId,
      selectedSprint.value.sprintId,
      retroNotes.value,
    );
...
async function createSprint() {
  try {
    await backlogService.createSprint(props.projectId, newSprint.value);
...
async function addTeam() {
  try {
    await backlogService.createTeam(props.projectId, newTeam.value);
...
async function deleteTeam(team: any) {
  if (confirm(`Delete team "${team.name}"?`)) {
    try {
      await backlogService.deleteTeam(props.projectId, team.id);
      await loadTeams();
    } catch (err) {
...
async function addRelease() {
  try {
    await backlogService.createRelease(props.projectId, newRelease.value);
...
async function deleteRelease(release: any) {
  if (confirm(`Delete release "${release.name}"?`)) {
    try {
      await backlogService.deleteRelease(props.projectId, release.id);
      await loadReleases();
    } catch (err) {
