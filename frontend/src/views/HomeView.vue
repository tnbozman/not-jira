  ...
  <div class="space-y-8">
    ...
    <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card class="cursor-pointer hover:shadow-md transition-shadow border border-surface-200" @click="router.push('/projects/new')">
          <template #content>
        ...
        <Card class="cursor-pointer hover:shadow-md transition-shadow border border-surface-200" @click="router.push('/projects')">
          <template #content>
            <div class="flex flex-col items-center text-center gap-3 py-2">
              ...
              <div>
                ...
                <p class="text-sm text-surface-500">{{ projects.length }} project{{ projects.length !== 1 ? 's' : '' }}</p>
              </div>
            </div>
          </template>
        </Card>
    ...
    <div v-if="projects.length > 0">
      <div class="flex items-center justify-between mb-4">
        ...
        <Button label="View All" icon="pi pi-arrow-right" iconPos="right" text size="small" @click="router.push('/projects')" />
      </div>
          ...
          <template #content>
            <div class="space-y-3">
              <div class="flex items-center gap-2">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-mono font-semibold bg-blue-100 text-blue-700">
                  {{ project.key }}
                </span>
              </div>
              <h3 class="font-semibold text-surface-800 text-lg">{{ project.name }}</h3>
              <p class="text-sm text-surface-500 line-clamp-2">{{ project.description || 'No description' }}</p>
